...
static void curx_miss_checker_insert(struct curx_miss_checker *c, unsigned int seq)
{
   c->seen[(c->seen_start + c->seen_size) & CURX_MISS_CHECKER_WINDOW_MASK] = seq;
   c->seen_size ++;
}

static void curx_miss_checker_remove_oldest(struct curx_miss_checker *c, struct curx_miss_checker_result *r)
{
   if(c->seen_size >= 2)
   {
